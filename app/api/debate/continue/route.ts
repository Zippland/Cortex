...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.messages.length > 1) {
      // 如果不是第一轮，提示AI回应对方的发言
      finalPrompt = `请你作为${currentAI.name}，针对辩题"${session.topic}"和上述对话内容，针对对方的漏洞有理有据地回击（包含具体翔实的证据、数据）。结构应当是：前一段回应对方的话，后一段打出自己的论点）。但是不管说什么，都要通过论据去论证出来，论据越详实可靠，说服力越大，不能直接瞎说。不要一直揪着同一件事情不放，如果同一个论据论点说了几次没效果就换打法，重点在于要怎么说服对方和裁判，注重辩论赛的观感。字数少一点，不要超过200字。`;
    } else {
      // 如果是第一轮，提示AI开始辩论
